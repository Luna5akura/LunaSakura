...
为了让你更有概念，这里是一个基础的数据结构设计：

```c
typedef struct {
    char* file_path;
    double start_time;    // 在时间轴上的开始时间 (秒)
    double duration;      // 持续时间
    double offset;        // 素材内部的偏移 (从素材的第几秒开始剪)
    int layer;            // 层级
    // 效果参数
    float opacity;
    float scale;
...
// 核心函数：获取某一帧的最终图像
// void* pixel_buffer 是输出的 RGB 数据
void render_frame(Project* p, double time, void* pixel_buffer);
```

```
Luna/
├── CMakeLists.txt          # 顶层 CMake，管理依赖（FFmpeg, SDL2）
├── main.c                  # 程序入口 (CLI 参数解析)
├── assets/                 # 测试用的视频/图片素材
│
├── include/                # 公共头文件
│   ├── common.h            # 全局宏、类型定义
│   ├── luna_vm.h           # 语言虚拟机对外接口
│   └── luna_video.h        # 视频引擎对外接口
│
├── src/
│   ├── vm/                 # [核心] 语言解释器 (原 core + builtin)
│   │   ├── chunk.c         # 字节码块
│   │   ├── compiler.c      # 编译器 (Parser -> Bytecode)
│   │   ├── scanner.c       # 词法分析 (原 lexer)
│   │   ├── vm.c            # 虚拟机主循环
│   │   ├── memory.c        # GC 和 内存管理
│   │   ├── object.c        # 字符串、函数等对象
│   │   ├── value.c         # 动态类型值
...
│   │
│   ├── engine/             # [核心] 视频渲染引擎
│   │   ├── timeline.c      # 时间轴数据结构 (Track, Clip)
│   │   ├── ffmpeg_dec.c    # FFmpeg 解码封装
│   │   ├── renderer.c      # 图像合成 (Blending, Resizing)
│   │   ├── audio_mixer.c   # 音频混音
│   │   └── exporter.c      # 导出视频文件
│   │
│   ├── binding/            # [胶水] 将 Engine 暴露给 VM
│   │   ├── bind_video.c    # 注册 "Clip", "Track" 等类到 VM
│   │   └── bind_math.c     # 注册数学函数
│   │
│   ├── player/             # [工具] 热更新预览播放器
│   │   ├── window.c        # SDL2/GLFW 窗口管理
│   │   └── preview.c       # 纹理上传与显示
│   │
│   └── utils/              # 通用工具
│       ├── file_io.c
