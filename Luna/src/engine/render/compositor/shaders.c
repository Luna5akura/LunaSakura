...
const char* FS_SOURCE_GLOW = "#version 330 core\n"
    ...
    "}\n"
    "vec3 extractGlow(vec4 c) {\n"
    " float knee = max(u_softness, 0.001);\n"
    " float t = smoothstep(u_threshold - knee, u_threshold + knee, luminance(c.rgb));\n"
    " return c.rgb * t * c.a;\n"
    "}\n"
    "void main() {\n"
    " vec4 src = texture(u_texture, TexCoord);\n"
    " if (u_radius <= 0.001 || u_intensity <= 0.001) { FragColor = src; return; }\n"
    " vec2 step1 = u_texel_size * u_radius;\n"
    " vec2 step2 = step1 * 2.0;\n"
    " vec2 step3 = step1 * 4.0;\n"
    " vec3 glow = extractGlow(src) * 0.18;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2( step1.x,  0.0))) * 0.11;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2(-step1.x,  0.0))) * 0.11;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2(0.0,  step1.y))) * 0.11;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2(0.0, -step1.y))) * 0.11;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2( step2.x,  step2.y))) * 0.08;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2(-step2.x,  step2.y))) * 0.08;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2( step2.x, -step2.y))) * 0.08;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2(-step2.x, -step2.y))) * 0.08;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2( step3.x,  0.0))) * 0.05;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2(-step3.x,  0.0))) * 0.05;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2(0.0,  step3.y))) * 0.05;\n"
    " glow += extractGlow(texture(u_texture, TexCoord + vec2(0.0, -step3.y))) * 0.05;\n"
    " vec3 tinted = mix(glow, glow * u_color.rgb, clamp(u_color.a, 0.0, 1.0));\n"
    " vec3 glowLayer = max(tinted * max(u_intensity, 0.0), vec3(0.0));\n"
    " vec3 result = 1.0 - (1.0 - src.rgb) * exp(-glowLayer);\n"
