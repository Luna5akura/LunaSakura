...
static void dictLiteral(bool canAssign) { // [新增]
    u8 pairCount = 0;
    if (!check(TOKEN_RIGHT_BRACE)) {
        do {
            if (check(TOKEN_RIGHT_BRACE)) break; // 允许尾随逗号
            expression(); // key
...
ParseRule rules[] = {
    ...
    [TOKEN_LEFT_BRACE] = {dictLiteral, NULL, PREC_NONE}, // [新增]
    [TOKEN_RIGHT_BRACE] = {NULL, NULL, PREC_NONE}, // [新增]
